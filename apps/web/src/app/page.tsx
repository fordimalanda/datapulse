...
// Format formaté pour vos composants graphiques (LiveMetricChart / Heatmap)
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    // Connexion au Gateway NestJS (port 3001)
    ...
    socket.on('connect', () => {
      ...
      // Souscription explicite aux métriques côté Gateway
    ...
    socket.on('metric_cpu_usage', (data: MetricPoint) => {
      const newMetric: FormattedMetric = {
        ...
        cpuUsage: data.value,
        memoryUsage: Math.floor(data.value * 0.85), // Simulé si souscrit à une seule métrique
